function info=kym_peakfitperkymographline(kymo,peakfitoption, psf_est,tresval)
posses=[];
info=struct('pos_frameno',[]);
[FramesNo,~]=size(kymo);
switch peakfitoption
    case 'peeling',cleankymo=kymo; 
    case 'just_treshold',
        sel=find(kymo>0);
        %sortkymo=sort(kymo(sel)); idx90=round(0.9*length(sortkymo));
        %tres=tresval*sortkymo(idx90);  %fraction of first 90%
            ...
            for jj=1:FramesNo 
                ...
                switch peakfitoption
                    case 'peeling'
                     ...
                     case 'flatbottom'
                       [firstpeaks,~, ~]=...
                        prf_get_peaks_on_flatbottom(prf,tresval,1,0);%data,sigs,refine,plotit
                    case 'just_treshold'
