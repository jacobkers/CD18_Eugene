function [AllPeakProps,AllClusterProps, AllClusters]=PeakPeeler(PeaksCurve,Psf,showit,PeakMode)
...
StopRelChange=0.01;     %0.01 means 1% of change in covered fraction
ChipIt=0.95;            %This is the fraction of the local maximum that ...
                        %is used to build the gauss to be subtracted
MainPeakMinFract=0.15;   %this sets the treshold for 'main peaks'; the number of these (not the positions) 
...
if nargin<4
    ...
    for ii=1:Pks
        ux=uxes(ii);
        PeaksCurve=PeaksCurve+OnePeak(xax,ux,sig,PeakMode);
    end
    %plot(xax,PeaksCurve);
end
...
while ~stopit
    ...
    OnePeakCurve=ChipIt*PeakVal*OnePeak(Xax,Xpos,Psf,PeakMode);    
