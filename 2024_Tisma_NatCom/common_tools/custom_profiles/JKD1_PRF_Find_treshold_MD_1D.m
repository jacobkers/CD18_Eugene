function [thr,xp]=JKD1_PRF_Find_treshold_MD_1D(data1D,show)
%JWJK_C*:-------------------------------------------------------------------
%Title: get treshold  
...
if nargin<2  %DEMO mode
    ...
    data1D=exp(-(r0/sig).^2)+exp(-(r1/sig).^2)+0.1*exp(-((y-r/2)/r).^2);
    data1D=(data1D-min(min((data1D)))).^4;    
    data1D=data1D+0.2*randn(r,c);
else
    [r,c]=size(data1D);
    [x,y]=meshgrid(1:c,1:r);
end%-------------------------------------------------------
