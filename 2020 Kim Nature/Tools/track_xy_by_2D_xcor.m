 function [x,y,prfx,prfy]=track_xy_by_2D_xcor(im); 
  ...
  if nargin<1  %DEMO mode
    ...
    im=MakeHighResRing(x0,y0,test);
     ...
     im=abs(im-nanmean(nanmean(im))); [r,c]=size(im);
      ...
      [xm,ym,~,~]=track_xy_by_com_2D_moment(im); 
      [imc,outofboundary_x]=matrix_pad_verticaledges(im,xm);
      [imt,outofboundary_y]=matrix_pad_verticaledges(imc',ym);
%      outofboundary_x=0;
%      outofboundary_y=0;
%imc=im;
     imc=imt';
